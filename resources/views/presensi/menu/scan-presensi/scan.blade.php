...
<body>
    ...
    function startQrScanner() {
        ...
        qrCodeScanner.start(
            ...
            (qrMessage) => { // Callback jika QR code berhasil di-scan
                if (!qrScanned) {
                    qrScanned = true;
                    // Setelah QR code berhasil dipindai, minta lokasi
    ...
    function onLocationSuccess(position, qrMessage) {
        ...
        // Kirim data ke server setelah mendapatkan lokasi
        $.ajax({
            url: '/attendance',
            type: 'POST',
            data: {
    ...
    function onLocationError(error) {
        ...
        qrScanned = false; // Reset agar bisa mencoba lagi
    }

    $(document).ready(function() {
        // Mulai pemindaian QR code saat halaman dimuat
        startQrScanner();
    });
</script>
